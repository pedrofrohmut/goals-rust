...
pub async fn add_user(client: &Client, user: &User) -> Result<(), UserDataAccessError>
{
    ...
    let stm = client.prepare(sql).await.map_err(|err| {
        UserDataAccessError::DbError(err)
    })?;

    client.execute(&stm, &[&name, &email, &phone, &password_hash]).await.map_err(|err| {
        UserDataAccessError::DbError(err)
    })?;
...
pub async fn find_user_by_email(client: &Client, email: &str) -> Result<Option<User>, UserDataAccessError>
{
    let str = "SELECT * FROM users WHERE email = $1";

    let stm = client.prepare(str).await.map_err(|err| {
        UserDataAccessError::DbError(err)
    })?;

    let rows = client.query(&stm, &[&email]).await.map_err(|err| {
        UserDataAccessError::DbError(err)
    })?;
    ...
    user.set_id(id.to_string()).map_err(|err| { UserDataAccessError::MappingError(err) })?;
    user.set_name(name).map_err(|err| { UserDataAccessError::MappingError(err) })?;
    user.set_email(email).map_err(|err| { UserDataAccessError::MappingError(err) })?;
    user.set_password_hash(password_hash).map_err(|err| { UserDataAccessError::MappingError(err) })?;
    user.set_phone(phone).map_err(|err| { UserDataAccessError::MappingError(err) })?;
